...
// 4タイプ診断
// ============================================================
const diagState = LS.get('diag', {view:'intro', answers:{}, currentQ:0, result:null});

function saveDiag() { LS.set('diag', diagState); }

function renderDiag() {
  const root = document.getElementById('diagView');
  if (diagState.view === 'intro') return renderDiagIntro(root);
  if (diagState.view === 'questions') return renderDiagQuestions(root);
  if (diagState.view === 'result') return renderDiagResult(root);
}

function renderDiagIntro(root) {
  root.innerHTML = `
    <div class="diag-hero">
      <h2>あなたの不調、どのタイプ？</h2>
      <p class="diag-sub">「異常なし」と言われ続けた体が整いだす、たった20問の体質診断</p>
      <p>柔道整復師・鍼灸師として <b>25,000人の不調</b> を診てきた冨高誠治が、東洋医学の体質分類から見つけた <b>「4つのタイプ」</b>。あなたの頭痛も、めまいも、慢性疲労も、<b>どのタイプかで、やるべき養生がまるで違う</b>んです。</p>
      <button id="diagStart" class="btn-primary">3分で診断を始める</button>
    </div>
    <h3 class="cat-head">体験者の声</h3>
    ${DIAG_VOICES.map(v => `
      <div class="voice">
        <div class="voice-head"><b>${v.name}</b><span class="tag">${v.type}</span></div>
        <p>${escapeHtml(v.body)}</p>
      </div>`).join('')}
    <p class="note">※ すべて個人の感想・体験談です。効果には個人差があります。</p>
    <details class="card" style="margin-top:14px;">
      <summary><b>診断の前に</b></summary>
      <p class="lead">20問・4択（よく当てはまる／やや／あまり／まったく）。所要時間3分。最後にタイプ（A〜D）と「体の声を聴く7日養生プログラム」が表示されます。</p>
    </details>
  `;
  document.getElementById('diagStart').addEventListener('click', () => {
    diagState.view = 'questions';
    diagState.currentQ = 0;
    diagState.answers = {};
...
function renderDiagQuestions(root) {
  const total = DIAG_QUESTIONS.length;
  const idx = diagState.currentQ;
  const q = DIAG_QUESTIONS[idx];
  const progress = Math.round((idx / total) * 100);

  root.innerHTML = `
    <div class="progress"><div class="progress-bar" style="width:${progress}%"></div></div>
    <p class="hint">Q${idx + 1} / ${total}</p>
    <div class="card" style="padding:14px 16px;">
      <p class="q-text"><b>${escapeHtml(q.text)}</b></p>
      <div class="diag-choices">
        ${DIAG_CHOICES.map((c, i) => `
          <button class="choice-btn" data-score="${c.score}">${c.label}</button>
        `).join('')}
      </div>
    </div>
    <div class="toolbar" style="margin-top:10px;">
      ${idx > 0 ? `<button id="diagPrev" class="btn-sub">← 前へ</button>` : ''}
      <button id="diagReset" class="btn-sub" style="margin-left:auto;">最初から</button>
    </div>
  `;
  root.querySelectorAll('.choice-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      diagState.answers[idx] = parseInt(btn.dataset.score, 10);
      if (idx + 1 >= total) {
        diagState.result = calcDiagResult();
        diagState.view = 'result';
      } else {
        diagState.currentQ = idx + 1;
      }
      saveDiag();
      renderDiag();
    });
...
function calcDiagResult() {
  ...
  const maxCount = {A:0, B:0, C:0, D:0}; // 「よく当てはまる」の数
  DIAG_QUESTIONS.forEach((q, i) => {
    const s = diagState.answers[i] ?? 0;
    scores[q.type] += s;
    if (s === 3) maxCount[q.type]++;
  });
  // タイブレーク: スコア → 「よく当てはまる」数 → A→B→C→D 優先
  const order = ['A','B','C','D'];
...
function renderDiagResult(root) {
  const r = diagState.result;
  const t = DIAG_TYPES[r.top];
  root.innerHTML = `
    <div class="result-card">
      <p class="note">あなたは…</p>
      <h2 style="margin:4px 0 6px;">タイプ${r.top}「${t.name}」</h2>
      <p class="lead"><b>${t.sub}</b>／キーワード：${t.keywords}</p>
      <div class="diag-scores">
        ${['A','B','C','D'].map(k => {
          const ds = DIAG_TYPES[k]; const sc = r.scores[k]; const max = 15;
          const pct = (sc/max)*100;
          return `<div class="diag-bar ${k===r.top?'top':''}">
            <span class="diag-bar-label">${k} ${ds.name}</span>
            <div class="diag-bar-track"><div class="diag-bar-fill" style="width:${pct}%"></div></div>
            <span class="diag-bar-score">${sc}/15</span>
          </div>`;
        }).join('')}
      </div>
    </div>
    ...
    <div class="toolbar" style="margin-top:14px;">
      <button id="diagRestart" class="btn-sub">最初からやり直す</button>
    </div>
  `;
  document.getElementById('diagRestart').addEventListener('click', () => {
    if (!confirm('診断を最初からやり直しますか?')) return;
    diagState.view = 'intro';
    diagState.answers = {};
    diagState.currentQ = 0;
...
function renderTypeDetail(t) {
  return `
    <h3 class="cat-head">あなたへのメッセージ</h3>
    <div class="card" style="padding:14px 16px;">
      ${t.message.map(m => `<p>${escapeHtml(m)}</p>`).join('')}
    </div>

    <h3 class="cat-head">あなたへの手紙（冨高誠治より）</h3>
    <details class="card" open>
      <summary><b>手紙を読む</b></summary>
      <div class="letter">${t.letter.split('\n').map(l => l ? `<p>${escapeHtml(l)}</p>` : '').join('')}</div>
    </details>

    <h3 class="cat-head">タイプの特徴</h3>
    <details class="card" open>
      <summary><b>${t.name}とは</b></summary>
      <div class="letter">${t.feature.split('\n').map(l => l ? `<p>${escapeHtml(l)}</p>` : '').join('')}</div>
      <h5 style="margin:8px 16px 4px;color:var(--accent);">このタイプに多い不調</h5>
      <table class="rev" style="margin:0 16px 12px;"><thead><tr><th>不調</th><th>東洋医学的な見方</th></tr></thead><tbody>
        ${t.symptoms.map(s => `<tr><td>${escapeHtml(s[0])}</td><td>${escapeHtml(s[1])}</td></tr>`).join('')}
      </tbody></table>
    </details>

    <h3 class="cat-head">今すぐできるセルフケア3つ</h3>
    ${t.care.map((c, i) => `
      <details class="card" open>
    ...
    <h3 class="cat-head">陥りやすい罠（NG養生）</h3>
    ${t.traps.map((tr, i) => `
      <details class="card">
        <summary><b>罠${i+1}：${escapeHtml(tr[0])}</b></summary>
        <p style="padding:0 16px 12px;">${escapeHtml(tr[1])}</p>
      </details>`).join('')}

    <h3 class="cat-head">食材リスト</h3>
    <details class="card" open>
      <summary><b>◯ おすすめ食材</b></summary>
      <table class="rev" style="margin:0 16px 12px;"><tbody>
    ...
    <h3 class="cat-head">季節ごとの注意点</h3>
    <details class="card">
      <summary><b>春夏秋冬の養生</b></summary>
      <table class="rev" style="margin:0 16px 12px;"><thead><tr><th>季節</th><th>注意</th><th>おすすめ</th></tr></thead><tbody>
        ${t.seasons.map(s => `<tr><td><b>${escapeHtml(s[0])}</b></td><td>${escapeHtml(s[1])}</td><td>${escapeHtml(s[2])}</td></tr>`).join('')}
      </tbody></table>
    </details>

    <details class="card" style="margin-top:14px;background:#fef9e7;">
